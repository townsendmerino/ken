...
const (
	...
	// generatedHeadBytes bounds how far into a file the generated-marker sniff
	// looks. Generated banners sit at the very top (often line 1, always within
	// the license header); bounding the scan keeps a stray "generated" mention
	// deep in real source from tripping the opt-in KEN_SKIP_GENERATED filter.
	generatedHeadBytes = 2048
)
...
func WalkFS(fsys fs.FS, opts Options) ([]string, error) {
	...
	skipGenerated := resolveSkipGenerated()
	...
	err := fs.WalkDir(fsys, ".", func(path string, d fs.DirEntry, err error) error {
		...
		if binary, minified, generated := sniffFS(fsys, path, maxAvgLine, skipGenerated); binary || minified || generated {
			return nil
		}
...
func (m *Matcher) ShouldIndex(relPath string) bool {
	...
	if binary, minified, generated := sniffOS(abs, m.maxAvgLineBytes, m.skipGenerated); binary || minified || generated {
		return false
	}
	return true
}
...
// resolveSkipGenerated reads KEN_SKIP_GENERATED (default OFF). When on, files
// whose header carries a machine-generated-code marker (see looksGenerated) are
// skipped at index time. Opt-in, not default-on: some corpora legitimately want
// their generated surface searchable (an agent asking "where is the typed
// client for X" wants the generated client-go file), so the filter is a
// deliberate choice the operator makes, not a silent default. Distinct from the
// KEN_MAX_AVG_LINE_BYTES minified sniff (which catches built bundles by line
// shape); this is a header-marker sniff for source that is generated but
// otherwise normally-shaped (protobuf stubs, client-go listers, mocks).
func resolveSkipGenerated() bool {
	switch strings.ToLower(strings.TrimSpace(os.Getenv("KEN_SKIP_GENERATED"))) {
	case "1", "true", "yes", "on":
		return true
	}
	return false
...
// looksGenerated reports whether a sampled file head carries a machine-
// generated-code marker: the tool-agnostic `@generated` tag, or a "Code
// generated … DO NOT EDIT" banner (Go's convention, also emitted by protoc,
// stringer, mockgen, client-go, and many others). Conservative on purpose — it
// requires the "do not edit" anchor to co-occur with a generated-by phrase, so
// a hand-written file that merely says "DO NOT EDIT below this line" is not
// flagged. Only the top generatedHeadBytes are scanned.
func looksGenerated(sample []byte) bool {
	head := sample
	if len(head) > generatedHeadBytes {
		head = head[:generatedHeadBytes]
	}
	// @generated is a standalone, tool-agnostic marker (Buck/Bazel, protoc,
	// many codegen tools) — strong enough on its own.
	if bytes.Contains(head, []byte("@generated")) {
		return true
	}
	lower := bytes.ToLower(head)
	if !bytes.Contains(lower, []byte("do not edit")) {
		return false
	}
	return bytes.Contains(lower, []byte("code generated")) ||
		bytes.Contains(lower, []byte("generated by")) ||
		bytes.Contains(lower, []byte("autogenerated")) ||
		bytes.Contains(lower, []byte("auto-generated")) ||
		bytes.Contains(lower, []byte("generated file"))
}
...
// binary (NUL byte, git's heuristic), minified (looksMinified), and — when
// skipGenerated is on — generated (looksGenerated). An unreadable file is
// reported binary so it isn't indexed. Real-FS variant for Matcher.ShouldIndex.
func sniffOS(path string, maxAvgLine int, skipGenerated bool) (binary, minified, generated bool) {
	f, err := os.Open(path)
	if err != nil {
		return true, false, false // unreadable ⇒ don't index
	}
	defer f.Close()
	...
	return false, looksMinified(b, maxAvgLine), skipGenerated && looksGenerated(b)
}

// sniffFS is the fs.FS variant of sniffOS, used by WalkFS. skipGenerated gates
// the (opt-in) generated-header check; the binary + minified checks always run.
func sniffFS(fsys fs.FS, path string, maxAvgLine int, skipGenerated bool) (binary, minified, generated bool) {
