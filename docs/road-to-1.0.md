...
| — | Perf campaign: startup + query latency | 🟡 in progress | — | Tracked in [`docs/perf-campaign-startup-query.md`](perf-campaign-startup-query.md). **M0 baselines** ([memo](../outputs/perf-startup-m0-baselines.md)) → **M2 lazy rerank load shipped** (2026-06-02, [memo](../outputs/perf-startup-m2-results.md)): cuts 491 ms from ken-mcp startup when `KEN_MCP_RERANK=on` — rerank-on startup is now indistinguishable from unset (~30 ms median). Next: M4 parallel `structural.Build` (target: save ~1.3 s on jekyll-scale corpora) → M1 Q8 rerank default. M3 + M5 killed by M0 data. |
