...
| HNSW | `github.com/townsendmerino/aikit/ann` ships HNSW as of aikit v0.2.0 (deferred — flat `internal/ann/flat.go` for v1) | Pure Go, same module ken already imports. The swap would be local — `internal/ann/` exposes only `Flat.Query(q, k) []Hit` today; the eventual interface is "to be introduced when the swap happens" rather than already-present scaffolding. Flat is exact and fine at repo scale. |
