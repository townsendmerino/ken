...
Engine scope is **Postgres + SQLite + MySQL as of v0.7.2**. The `internal/db.IndexSchema` entry point dispatches on the DSN scheme: `postgres://` / `postgresql://` → `indexSchemaPostgres` (the v0.7.0 path); `sqlite://` / `sqlite3://` → `indexSchemaSQLite` (sibling file `sqlite.go`, ADR-018, pure-Go `modernc.org/sqlite` driver, no cgo); `mysql://` or the native go-sql-driver form (detected by `@tcp(` / `@unix(` substring) → `indexSchemaMySQL` (sibling file `mysql.go`, ADR-019, pure-Go `github.com/go-sql-driver/mysql` driver). The `Options` struct + `Refresher` + `WatchedIndex.SetExtraChunks` machinery is engine-agnostic and reused unchanged. v0.7.2 also adds **schema filtering** for Postgres + MySQL via `KEN_DB_SCHEMAS` (allow-list) and `KEN_DB_EXCLUDE_SCHEMAS` (deny-list); SQLite is single-schema and ignores them with a debug log. The canonical filter source is `internal/db.filterSchema`, applied per-row in each engine's introspection path so default exclusions (`pg_catalog`, `information_schema`, `mysql`, `performance_schema`, `sys`) are always honored regardless of user lists; both env vars set → allow-list wins with a stderr warn. See ADR-017 for the v0.7.0 alternatives considered, [ADR-018](DECISIONS.md#adr-018-sqlite-engine--migration-history-folding-via-lightweight-alter-replay) for the v0.7.1 SQLite + migration-folding additions, and [ADR-019](DECISIONS.md#adr-019-mysql-engine--schema-filtering-for-multi-schema-dev-databases) for the v0.7.2 MySQL + filtering additions (rejected alternatives: replace-vs-extend default exclusions, wildcards deferral, intersection-vs-allow-list-wins, per-table filtering, MariaDB-as-separate-engine).

v0.7.1 also adds **Tier-1 migration-history folding** (ADR-018) — when `internal/search`'s walker detects a directory of numbered `.sql` migration files (Goose / Rails / Flyway / Alembic patterns), `internal/sql.FoldMigrations` replays CREATE TABLE + later ALTER TABLE statements into one "current state" chunk per table. The lightweight approach (parse ALTER, mutate in-memory column list, emit folded chunk) covers ADD COLUMN / DROP COLUMN / ALTER COLUMN TYPE / ADD CONSTRAINT / DROP CONSTRAINT. Unfoldable statements (RENAME COLUMN, type conflicts, missing CREATE TABLE) emit BOTH the original per-file chunk AND the folded chunk so the agent never sees less information than v0.7.0. Opt-out via `KEN_SQL_NO_AUTO_MIGRATIONS=1`.
