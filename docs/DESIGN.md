...
- **Tree-sitter chunker (Option A).** **Status (v0.2.0): landed via [`gotreesitter`](https://github.com/odvcencio/gotreesitter), shipping as opt-in (`--chunker=treesitter` / `KEN_MCP_CHUNKER=treesitter`); default stays `regex`.** Measured hybrid NDCG: 0.838 (Δ −0.004 vs the regex baseline 0.842 — within noise). Per-language wins: kotlin +0.011, zig +0.013, typescript +0.009, java +0.006, php +0.005. Losses: python −0.009, c −0.017, rust −0.013, lua −0.022. Most languages within ±0.005. The chunker pivot (originally WASM/wazero) is recorded in ADR-010; the default-stays-regex decision is ADR-011. Both grammars have known issues skipped via `kenToTreeSitter`:
  ...
  - **Swift grammar** misparses real-world Swift in the structural extractor pass. The chunker layer keeps the Swift grammar (it still produces some boundaries, even from ERROR roots); the structural extractor needs a clean AST and gets a `root=ERROR` parse instead. **Tested 2026-06-03 against gotreesitter `v0.20.0-rc3`:** the lexer fails to terminate `//` line comments past trivial content — even a 4-line file whose only header is `// software` or `// and` produces a `root=ERROR` parse. Cross-corpus survey: 0% clean on Alamofire, 2% on swift-nio, 8% on swift-collections, 35% on sindresorhus/Defaults — universally broken on shipping code. **Re-tested 2026-06-06 against gotreesitter `v0.20.2`** (#99/#107, "recover Swift top-level declarations after license headers"): the license-header fix lifted Alamofire **0% → 35% clean**, but it is **still not production-viable** — ~65% of real Swift files still parse to a non-clean root, and ~20% take **2–6 seconds each** (pathological recovery; with the structural pool's `parseTimeoutMicros=0` an unbounded file hangs the indexer). Swift **stays parked** behind the `swift` build tag at [`internal/structural/extract_swift.go`](../internal/structural/extract_swift.go); re-enabling requires (a) `.swift → swift` in `kenLangToTSLang`, (b) `swift → extractSwift` in `langExtractor`, (c) dropping the build tag. **Trigger to revisit:** a further `gotreesitter` Swift release that brings clean-parse well above 35% and removes the multi-second recovery cost. Diagnostic memo at [`docs/internal/swift-parse-root-cause.md`](internal/swift-parse-root-cause.md).
