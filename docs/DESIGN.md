...
Engine scope is **Postgres only for v0.7.0**. MySQL and SQLite share `internal/db`'s shape (the engine-specific bit is just `introspect.go`'s queries) and land in follow-on point releases. See ADR-017 for the alternatives considered (column-exclusion DSL, per-call introspection, LISTEN/NOTIFY, agent-triggerable reindex tool, `mcp.Run` DB support — all deferred or rejected).
