...
- **`internal/db`** — live Postgres introspection via `github.com/jackc/pgx/v5` (pure Go, default Tracer is nil so no stdout-pollution risk). `IndexSchema(ctx, opts)` runs the per-category queries (tables + columns, constraints, indexes, FK references in both directions, views, function signatures) and emits chunks with a freshness header. `Refresher` orchestrates the three reindex layers (startup once, periodic via `Options.ReindexInterval`, manual via `Refresh`); all three share one mutex-serialized `IndexSchema` call and reuse ADR-012's atomic snapshot-swap via `WatchedIndex.SetExtraChunks`.
...
Engine scope is **Postgres + SQLite as of v0.7.1**. The `internal/db.IndexSchema` entry point dispatches on the DSN scheme: `postgres://` / `postgresql://` → `indexSchemaPostgres` (the v0.7.0 path); `sqlite://` / `sqlite3://` → `indexSchemaSQLite` (sibling file `sqlite.go`, ADR-018, pure-Go `modernc.org/sqlite` driver, no cgo). The `Options` struct + `Refresher` + `WatchedIndex.SetExtraChunks` machinery is engine-agnostic and reused unchanged. MySQL lands in v0.7.2 via the same engine-dispatch shape. See ADR-017 for the alternatives considered (column-exclusion DSL, per-call introspection, LISTEN/NOTIFY, agent-triggerable reindex tool, `mcp.Run` DB support — all deferred or rejected) and [ADR-018](DECISIONS.md#adr-018-sqlite-engine--migration-history-folding-via-lightweight-alter-replay) for the v0.7.1 SQLite + migration-folding additions.
