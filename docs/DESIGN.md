...
- **HNSW for the dense retriever.** ken uses `aikit/ann.Flat` today — float32-SIMD cosine (near-exact), fine at repo scale. The flat retriever exposes only `Flat.Query(q, k) []Hit`; a `Retriever`-style interface would be introduced when the swap lands, not preemptively. **Trigger:** dense matrix size makes exact cosine the bottleneck on a real workload. **Status (aikit v0.2.0, 2026-06-03):** the swap target now sits next to Flat in the same `aikit/ann` package — so when the trigger fires, the change is "switch the constructor in `internal/search/index.go` from `ann.New(...)` to the HNSW equivalent" plus the small Retriever interface around `Query`, no new dependency.
