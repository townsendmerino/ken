...
- **NDCG vs semble: measured at v0.1.0** — hybrid 0.842 vs semble 0.854 (Δ 0.012, 63 repos × 1251 queries). Semantic raw matches within 0.003 (validates algorithm port); BM25 tokenizer also brought to verbatim semble parity. Residual gap concentrated in non-Python supported languages and chunker-attributable (§10).

## Decisions

- **cgo: not allowed.** Pure Go. Single static binary, free `GOOS`/`GOARCH` cross-compile, no `libtokenizers.a` to vendor per platform.
- **Chunking: all three options behind a `Chunker` interface**, selectable at runtime. v1 ships Option C (hand-rolled per-language regex); B (Chroma) and A (WASM tree-sitter) are documented future paths (§2, §10).
- **Tokenizer: hand-rolled WordPiece** in `internal/embed/tokenize.go`, ~400 LOC, no external tokenizer dependency. Validated against `transformers.AutoTokenizer` on 11k+ inputs (§3).
...
### Option A — Tree-sitter via WASM (deferred)

**Parity:** Highest. Same parser, same grammars, same chunk boundaries as Chonkie.
**Cost:** Slower than native tree-sitter. Benchmarks suggest 3–10× slowdown depending on grammar. WASM grammar binaries are 100 KB–2 MB each — bundling 19+ languages adds 10–30 MB to the binary (or load them lazily from a cache directory).
**Build path:** [`tetratelabs/wazero`](https://github.com/tetratelabs/wazero) (mature pure-Go WASM runtime) + tree-sitter's official WASM build of the parser core + per-language `.wasm` grammars from `tree-sitter-<lang>` releases.
**Verdict:** The "maximum parity with Python semble" option, for users who'll accept the binary size and latency cost. The right choice for benchmarking and ground-truth evaluation of B and C. See §10 for the trigger.
...
- **WASM tree-sitter chunker (Option A).** Highest parity; bigger binary; slower. **Trigger:** regex chunker's NDCG measurably below tree-sitter-based on a benchmark we can run. **Status (v0.1.0): triggered.** Measured on semble's benchmark: hybrid gap is 0.012 overall (0.842 vs 0.854) and is concentrated in non-Python supported languages: go −0.046, rust −0.050, zig −0.046, cpp −0.019, typescript −0.030, java −0.020. Python at +0.003 confirms the rest of the pipeline ports cleanly; the chunker is the remaining lever. Confirmed empirically: bringing the BM25 tokenizer to verbatim semble parity (snake-case compound preservation, ASCII-only run extraction) moved hybrid by only +0.002, ruling out the tokenizer as the dominant cause.
