...
- **External benchmark — CoIR-CSN-Python (v0.2.0).** ken evaluated against [CoIR](https://github.com/CoIR-team/coir)'s `CodeSearchNet-python` task: hybrid 0.7839, bm25 0.8743, semantic 0.7405 (1000-query subsample, regex chunker). Surprise worth recording: **on CSN-Python, BM25 outperforms hybrid by 0.09 — opposite of semble's bench.** Not a ken bug; CSN queries are full docstrings where lexical overlap with the answer function is high, so BM25 with identifier-aware tokenization is near-optimal and α=0.5 RRF fusion drags hybrid down. The α-weighted RRF fusion strategy (ported verbatim from semble) is therefore corpus-sensitive — works well on semble's bench (mixed conceptual / symbol queries), works against us on a docstring-heavy benchmark. **Possible v0.3+ investigation:** corpus-adaptive α (per-query auto-detect already exists for symbol-vs-NL; maybe also detect "docstring-shaped" queries and lean harder on BM25). Not currently triggered — published as an honest finding, not a regression. Full data in `docs/BENCH.md`.
...
- **Incremental indexing.** Today every `ken` invocation does a full walk → chunk → BM25-build → embed-all from scratch; ken-mcp caches the built index in-process via its LRU but does not invalidate on file changes (so a running ken-mcp serves a stale index until the LRU evicts that entry or the process restarts). True incremental indexing would require: (1) per-file change detection (mtime or content-hash), (2) `bm25.Index.Update` to add/remove postings when a chunk changes (today the postings map is built once and treated as immutable), (3) `ann.Flat.Update` to insert/replace embeddings at a chunk-index (today the dense matrix is fixed at construction), (4) a persisted representation so updates survive across processes — ties into the on-disk cache item above. **Bonus opportunity:** `gotreesitter` supports incremental reparsing and benchmarks faster on it than full parses (~666 ns vs ~2 ms), but the treesitter chunker currently discards the parse tree as soon as cAST extracts spans; keeping the per-file tree around would let us reparse only changed byte ranges. **Trigger:** users running ken-mcp on a repo they're actively editing report stale results, OR full-rebuild latency on a large corpus becomes the dominant per-query cost.
