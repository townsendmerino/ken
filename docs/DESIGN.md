...
- **ken 1.0.0 shipped + tagged 2026-06-06.** Public API frozen; 13 languages; structural tools; DB-schema indexing; opt-in neural reranker; model auto-fetch; macOS/Linux/Windows binaries. This §Status records the original build order; the authoritative current-state map is [ARCHITECTURE.md](../ARCHITECTURE.md) + the [CHANGELOG](../CHANGELOG.md).
...
- **Tokenizer parity: 11,447-input corpus-scale harness** (now in aikit: `scripts/parity_dump.py` + `embed/parity_test.go` under build tag `parity`, post-ADR-034) reports 0 drift across `normalize` / `pre_tokenize` / `wordpiece` / `other`. Surfaced and fixed three real bugs the spot-check had missed (see §3).
...
> **⚠️ Historical (pre-ADR-034).** The tree below predates the aikit extraction: `internal/chunk`, `internal/embed`, `internal/ann`, and `internal/bm25` now live in the external [aikit](https://github.com/townsendmerino/aikit) module (as `chunk`, `embed`, `ann`, `bm25` — plus `encoder`, `topk`, `fuse`). For the current layout, see **[ARCHITECTURE.md](../ARCHITECTURE.md)** — it is the authoritative map; this section is kept as design history.
...
| HNSW | `github.com/townsendmerino/aikit/ann` — ken consumes this same package today via `ann.Flat`; HNSW landed alongside in aikit v0.2.0. | Pure Go, same package ken already imports. The future swap is a constructor change (`ann.New(...)` → an HNSW equivalent) rather than a library import, gated on the trigger in §10. Flat scoring is float32-SIMD (`linalg.Dot`, near-exact — reorders only sub-1e-6 ties) and fine at repo scale. |
...
- Chunker side-effect registration moved out of `internal/search` into the binaries that want each optional chunker. `internal/search` keeps the `regex` (universal default) blank-import; `cmd/ken` and `cmd/ken-mcp` add `treesitter` + `markdown`; `cmd/ken-mcp-docs` adds only `markdown`. This is what keeps **~26 MB of binary inflation** out of the docs binary — the cost of importing `internal/chunk/treesitter` against `gotreesitter v0.18.0`: the `gotreesitter/grammars` embed.FS payload is ~19 MB on-disk for 206 `.bin` blobs, and the linked binary grows by ~26 MB once you add parser runtime + symbol overhead (darwin/arm64; measured by building `cmd/ken-mcp` with and without the blank import). Per-binary chunker selection. Per-grammar selection was *not* possible through gotreesitter v0.19.x — the embed bundle was monolithic at the source layer, per [ADR-023](internal/DECISIONS.md#adr-023-gotreesitter-grammar_subset-machinery--binary-size-reduction-outcome-v082-investigation-outcome). **As of [ADR-033](internal/DECISIONS.md#adr-033-adopt-gotreesitter-grammarsubset-slim-release-binaries-v0200-rc2) (gotreesitter v0.20.0-rc2) it is**: ken's release binaries build slim via `grammar_subset` tags, embedding only the 24 dispatched grammars (~14 MB lighter), while the library `go build` stays all-grammars.
...
Design rationale and the five locked sub-decisions are in [`DECISIONS.md` ADR-012](internal/DECISIONS.md#adr-012-incremental-indexing-via-fsnotify--atomic-snapshot-swap); the implementation lives in `internal/search/watch.go`.
...
- `THIRD_PARTY_LICENSES.md` — generated listing of every Go-module dep + its license (regenerate via `go run ./tools/gen-licenses`). The MPL-2.0 entry (`go-sql-driver/mysql`) is satisfied here: the upstream module's `LICENSE` file ships with the module via `go mod`, and the generated table preserves the SPDX identifier.
...
`cmd/ken-mcp` is a drop-in replacement for semble's MCP server. The two semble-compatible tools (`search`, `find_related`) keep arg shapes ported verbatim from `/tmp/semble/src/semble/mcp.py` and the same wire format (a formatted markdown string via `_format_results`), so agents already trained against semble work unchanged. ken adds eight more — the structural tools (`definition`, `references`, `callers`, `outline`, `symbols`), `status`, `recently_changed`, and `reindex_db` — for ten total (`mcp.Run` embedded-corpus builds expose the `search`/`find_related`/`status` subset).
...
- **Tree-sitter chunker (Option A).** **Status (v0.2.0): landed via [`gotreesitter`](https://github.com/odvcencio/gotreesitter), shipping as opt-in (`--chunker=treesitter` / `KEN_MCP_CHUNKER=treesitter`); default stays `regex`.** Measured hybrid NDCG: 0.838 (Δ −0.004 vs the regex baseline 0.842 — within noise). Per-language wins: kotlin +0.011, zig +0.013, typescript +0.009, java +0.006, php +0.005. Losses: python −0.009, c −0.017, rust −0.013, lua −0.022. Most languages within ±0.005. The chunker pivot (originally WASM/wazero) is recorded in ADR-010; the default-stays-regex decision is ADR-011. Both grammars have known issues skipped via `kenToTreeSitter`:
  - **C# grammar** — **RE-PARKED 2026-08-24 on gotreesitter `v0.51.0`** for a *second, unrelated* defect in the same grammar. Parsing a collection initializer built from `{ typeof(T), X.Instance }` entries costs time that grows explosively with the entry count: MessagePack's `BuiltinResolver.cs` (11 KB, 211 lines of **valid** C#) takes **250 s** and still returns `root=ERROR`; six files in that one repo exceed 30 s, with a 58-file tail over 1 s. Reproduced against raw gotreesitter with no ken code in the path, so it is the grammar rather than `extractCsharp`. Filed upstream as [odvcencio/gotreesitter#972](https://github.com/odvcencio/gotreesitter/issues/972). A synthetic `Dictionary<Type, object>` initializer reproduces the scaling — parse time doubles roughly every 8 entries — though it plateaus around 8.8 s, so the real file's cost is not fully explained by entry count alone. **This is not the v0.20.x OOM:** RSS stays flat (~400 MB) while a core spins, so neither the old memory symptom nor the 64 KiB size guard (the file is 11 KB) catches it. Impact was user-facing — `ken index` over a directory containing such a file appeared to hang (6 files × ~4 min each); `ken-mcp` was never exposed because it sets a 500 ms parse budget. **Mitigation:** `.cs` is removed from `kenLangToTSLang` (deterministic, and matching the standing precedent for the bash grammar) rather than given a wall-clock timeout, which would make enrichment load-dependent and trade a hang for the reproducibility bug ADR-040 closed. `extractCsharp` stays registered and tested (`TestBuild_CsharpBasics` drives it directly), so re-enabling is one line; `TestCsharpIsParkedFromTheLangMap` fails loudly if that line returns before upstream is fixed. Prior history below.
  - **C# grammar (prior OOM)** — **RESOLVED + shipped 2026-06-06 on gotreesitter `v0.20.2`.** Previously OOM'd on real-world C# (1.7+ GB RSS during dapper indexing → SIGKILL on all 3 csharp bench repos; v0.20.0-rc3 retest: 93+ GB before SIGKILL). gotreesitter v0.20.2 (#98/#106) **bounded the C# namespace-recovery sub-parses** whose unbounded recursion was the root cause (see [`docs/internal/archive/csharp-oom-root-cause.md`](internal/archive/csharp-oom-root-cause.md)). Re-verified on v0.20.2: Dapper's 156 `.cs` files parse in ~3s, 89% clean root, no OOM; the minimal reproducer now parses in ~5ms. C# was fully wired at that point (`.cs → c_sharp` in `kenLangToTSLang`, `c_sharp → extractCsharp` in `langExtractor`, `csharp → c_sharp` in aikit `KenToTreeSitter`, `grammar_subset_c_sharp` in the slim release build) — the map row has since been removed by the re-park above and the [`extract_csharp.go`](../internal/structural/extract_csharp.go) extractor is in the default build with a regression-guard test ([`extract_csharp_test.go`](../internal/structural/extract_csharp_test.go)) that exercises the former OOM trigger.
  ...
  - **Swift grammar — un-parked 2026-08-19 on gotreesitter `v0.48.1`.** History: on `v0.20.0-rc3` the Swift lexer failed to terminate `//` comments past trivial content, so even a 4-line file with a `// software` header parsed to `root=ERROR` (0% clean on Alamofire, 2–35% across swift-nio/collections/Defaults); `v0.20.2` (#99/#107) lifted Alamofire to 35% but left ~65% non-clean and ~20% taking **2–6 s each** — not viable, so Swift stayed parked behind a `swift` build tag. **On the pinned `v0.48.1` that failure class is gone:** a real-corpus survey (Alamofire `Session.swift`/`Request.swift`/…, swift-collections) parses to `root=source_file`, `stop=accepted`, in **26–260 ms** with no crash/hang. Newer-syntax files still carry *localized* ERROR nodes (the grammar isn't fully current with Swift's evolution — e.g. Request.swift has scattered errors), but the parser recovers, so the extractor degrades to a **partial-but-additive** label rather than failing. Swift is now registered in `kenLangToTSLang` + `langExtractor` (the `swift` build tag dropped); [`extract_swift.go`](../internal/structural/extract_swift.go)'s v0.20-era node-type mapping was re-verified against v0.48.1 by `TestBuild_SwiftBasics`. **Residual:** on the `ken build-index` path specifically (no wall-clock parse budget, by design — [ADR-044](internal/DECISIONS.md#adr-044-cli-default-enrichment-parse-budget-build-index-excepted)) a pathological Swift file adds unbounded structural-parse cost; `ken index`/`search`/`bench`/`perf` default to a 2s budget and ken-mcp to 500ms as of ADR-044 (2026-09-07) — prompted by finding the *same failure class* on plain `.c`/`.h` files at real scale, not Swift specifically (see [`gotreesitter-c-large-init-array-slowdown.md`](internal/gotreesitter-c-large-init-array-slowdown.md)). **Scala + Elixir** structural extractors were added in the same batch (good/medium fit); Zig/Lua/Haskell remain chunker-only (the `func/calls/raises/classes` model fits them poorly — deferred pending a measured NDCG case).
...
- **Dense retriever beyond `ann.Flat` — RESOLVED 2026-09-06, `HNSW` declined / `FlatBinaryI8` adopted (opt-in).** ken used `aikit/ann.Flat` alone through v1.x — float32 cosine, fine at repo scale, but the named trigger ("dense matrix size makes exact cosine the bottleneck on a real workload") was real: measured at N=800k, `Flat`'s query is 9ms vs sub-2ms for the quantized alternatives (see below). Full measurement + decision in [`docs/internal/dense-retriever-adoption-2026-09.md`](internal/dense-retriever-adoption-2026-09.md) (roadmap A2; [`DECISIONS.md` ADR-043](internal/DECISIONS.md#adr-043-dense-retriever-seam--optin-flatbinaryi8-hnsw-declined)). The small `Retriever`-style interface anticipated here shipped as the unexported `denseRetriever` (`internal/search/index.go`, `Query(q, k) []ann.Hit`) — every candidate in `aikit/ann` already satisfies it, so no new dependency was needed. **`HNSW` is declined**, not adopted: its query is competitive from ~50k vecs on, but build time reproduces a 4m23s@200k anchor almost exactly (271.8s measured) — incompatible with `ken index --watch`'s 2-second republish contract — and it keeps the full f32 vectors *plus* a graph, so it costs *more* memory than `Flat`, not less. **`ann.FlatBinaryI8`** (binary Hamming prefilter + int8 rerank) is adopted as an opt-in for **repo/mid scale**: zero measured end-to-end recall cost on the real 63-repo/1251-query semble benchmark (pipeline recall@10 identical to 3 decimals vs `Flat`), fastest at every scale measured. **`ann.FlatI8`** is the pick for **real kernel scale**: a same-day follow-up spot-check against a real 584k-chunk Linux kernel checkout (`internal/search/kernel_scale_spotcheck_test.go`) found `FlatI8` holds there (agree@10 vs exact `Flat` ≈1.00) while `FlatBinaryI8` does not (0.96) — real code's semantic clustering at real scale stresses the binary prefilter in a way neither the repo-scale real corpora nor the synthetic scale ramp could reveal. Wired as `FSOptions.DenseRetriever` / `KEN_ANN=flat-i8|flat-binary-i8`, default unchanged (`Flat`) at every scale — see [ADR-043](internal/DECISIONS.md#adr-043-dense-retriever-seam--optin-flatbinaryi8-hnsw-declined)'s update note for the full revision.
...
- **Persistent on-disk index cache — SHIPPED in v1.3.0 ([ADR-039](internal/DECISIONS.md#adr-039-persistent-index-snapshot--reconcile-on-boot-cold-start-m1)).** `ken-mcp` persists the built index to `<repo>/.ken/snapshot.{bin,manifest}` and, on restart, loads + drift-scans (config-key ⊕ per-file mtime/size) instead of rebuilding when the corpus is unchanged across processes, reconciling only changed files otherwise (`cmd/ken-mcp/snapshot.go`; `KEN_MCP_SNAPSHOT`, default on; any missing/corrupt/mismatched/drifted state falls back to a live build). Distinct from incremental indexing below — this skips a rebuild *across* processes; incremental indexing handles in-process file-level deltas. (Serve-before-warm opt-in modes — `KEN_MCP_STAGED` / `KEN_MCP_LAZY_ENRICH` / `KEN_MCP_EMBED_CACHE` — layer on top.)
- **Incremental indexing — landed in v0.3 via fsnotify + atomic snapshot swap.** `internal/search/watch.go` wraps an `*Index` in a `WatchedIndex` whose `atomic.Pointer[Index]` is published anew every 2 seconds of edit activity. Readers do one atomic load per Search/FindRelated/ResolveChunk call; writers build a new snapshot off to the side. No reader-side lock; in-flight queries see consistent snapshots. ken-mcp watches always; `ken index --watch` is the default (`--no-watch` is the v0.2-compatible opt-out). Tombstone-then-compact: deleted files leave their chunks in the slice with `Tombstoned=true` and query paths skip them, then they are **compacted out on every flush** (`WatchedIndex.compactCorpus`, unconditional — ADR-012), so memory no longer grows monotonically with cumulative edit volume. **The v0.3.x compaction trigger is resolved.** **Bonus opportunity (still deferred):** `gotreesitter` supports incremental reparsing (~666 ns vs ~2 ms), but the treesitter chunker still discards the parse tree per file. Keeping the tree around would let us reparse only changed byte ranges — orthogonal to v0.3's incremental indexing, would land as v0.3.x optimization. See [`DECISIONS.md` ADR-012](internal/DECISIONS.md#adr-012-incremental-indexing-via-fsnotify--atomic-snapshot-swap) for the full design.
...
- **Function-level resolved call graph + transitive `impact` tool.** Today's `callers(symbol)` returns the FILES containing a call to *some* symbol of that name; the corpus-wide reverse index is name-resolved, not function-resolved. The deferred upgrade resolves each call site to the specific calling function/method, ranks edges by confidence, and adds a bounded transitive-traversal tool (`impact(symbol, depth)` — "what could break if I change this"). **Phase 0 (substrate) shipped 2026-06-03** — span fields on `FuncDef`/`ClassDef`, per-call-site `CallRef` records with `Callee` / `Receiver` / `Line` / `EnclosingSymbol` attribution, `CalleeNames()` accessor preserving Arm B byte-identity. Phases 1, 2, 4 (resolution + dependency edges + `impact` tool) are bundled behind a `KEN_STRUCTURAL_GRAPH=on` opt-in flag pending user-signal evidence; Phase 3 (type/heritage resolution) is hard-deferred. Full plan + per-phase performance budgets + the validation-harness scope + the silent-wrong-answer watch-mode risk + the Plan-agent independent review are in [`docs/internal/structural-call-graph-plan.md`](internal/structural-call-graph-plan.md). **Trigger to ship Phases 1+4:** MCP log evidence that the agent's current 2-step `callers` → `outline` → re-query pattern is in practice 3+ steps in a measurable fraction of `callers` invocations (rule of thumb: >30%). One week of `ken-mcp` logs settles the question.
