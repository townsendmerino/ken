...
| Query-time graph expansion (additive boost) | Killed | [results/stage8-qgraph-expansion-results.md](results/stage8-qgraph-expansion-results.md) |
| ColBERT MaxSim cheap-reuse on CodeRankEmbed token vectors | Parked (slim N=25, both prefix-on and prefix-off) | [results/stage8-maxsim-probe-parked.md](results/stage8-maxsim-probe-parked.md). Companion analysis at [colbert-late-interaction-for-ken.md](colbert-late-interaction-for-ken.md). |
...
| — | Windows binary | 🟢 shipped 2026-06-06 | done | Re-opened 2026-06-05 (was ⚪ deferred); shipped as part of v0.10.0. `.goreleaser.yml` now targets `windows/amd64`+`arm64` (`.zip` archives); `sighup_windows.go` already no-ops the Unix-only SIGHUP path (the `aikit v0.4.1` Windows build fix landed earlier). Release-equivalent slim builds cross-compile clean both arches; `goreleaser check` green. The scoop manifest (v0.10.1) + the `windows-latest` CI smoke job shipped 2026-06-06 too, so the C3 distribution set is complete. |
...
| 5 | `recently_changed(N)` MCP tool (git-aware) | 🟢 shipped | done | 2026-06-02. mcp/recently_changed_tool.go — go-git PlainOpen, walks HEAD N commits back, formats commit + changed-file list as markdown. Args: `n` (default 10, max 100), `repo`, `path` prefix filter. **URL repos supported 2026-08-15**: resolved through the cache to the clone working tree via `WatchedIndex.Root()` (not by recomputing temp paths); the depth-1 shallow clone limits history to the tip commit(s), surfaced as an honest note (JSON `note` field + markdown caveat). |
...
| — | Perf campaign: startup + query latency | 🟢 closed (ADR-036) | done | 2026-06-02. **M0 baselines** ([memo](results/perf-startup-m0-baselines.md)) → **M2 lazy rerank load** ([memo](results/perf-startup-m2-results.md)): −491 ms when `KEN_MCP_RERANK=on`. → **M4 parallel `structural.Build`** ([memo](results/perf-startup-m4-results.md)): 3.5× on jekyll (−1,127 ms). → **M1 killed without code change** ([memo](results/perf-startup-m1-results.md)): M2 superseded it. M3 + M5 killed by M0 data. **Cumulative cold-start reduction: 55-79% across corpora**; warm-search p50 already sub-ms. Closed via [ADR-036](DECISIONS.md#adr-036). |
| — | Language coverage (post-1.0 ship list) | 🟢 13 languages shipped | done | 2026-06-03 (v0.9.1): Kotlin + Dart on top of the v0.9.0 ten (Python · Go · TypeScript · JavaScript · Java · Rust · C · C++ · PHP · Ruby). **C# added 2026-06-06** on gotreesitter v0.20.2 — the OOM that parked it (#98/#106 bounded the C# namespace-recovery sub-parses) is fixed upstream; coordinated the same three wirepoints as Dart (aikit `chunk/treesitter` v0.4.1 `KenToTreeSitter` + `.goreleaser.yml` `grammar_subset_c_sharp` + structural extractor), drift-guard green, with an OOM-trigger regression test. **Swift stays parked**: re-tested on v0.20.2 (#99/#107 license-header fix lifted Alamofire 0%→35% clean, but ~65% still fail + ~20% take 2–6s/parse — not viable). Diagnostic memos: [docs/internal/csharp-oom-root-cause.md](archive/csharp-oom-root-cause.md) (now resolved) + [docs/internal/swift-parse-root-cause.md](swift-parse-root-cause.md). Developer walkthrough at [docs/internal/add-a-language.md](add-a-language.md). |
...
- 🟢 **Bulk-search** — shipped 2026-08-15. `search` `queries` arg runs a batch in one call; per-query grouping (JSON `{queries:[…]}` / markdown sections), shared `searchCore`, capped at 20.
- 🟢 **Per-result "why this match" explanation** — shipped 2026-08-15 as the opt-in `explain` arg. Lexical-overlap scope (which query terms appear in the chunk, kind=lexical/semantic), NOT full stage-by-stage provenance — that "hard" version would need per-result pipeline instrumentation.
...
- 🟢 **Hybrid-by-default onboarding — COMPLETE 2026-06-06.** Opened 2026-06-05. The recall
