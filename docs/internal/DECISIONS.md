...
| [ADR-043](#adr-043-dense-retriever-seam--optin-flatbinaryi8-hnsw-declined) | Dense-retriever seam — opt-in `FlatBinaryI8`, `HNSW` declined | Accepted |
| [ADR-044](#adr-044-cli-default-enrichment-parse-budget-build-index-excepted) | CLI default enrichment parse budget (`build-index` excepted) | Accepted |
| [ADR-045](#adr-045-ken-mcp-mmaps-the-embedding-model-cli-keeps-the-heap-read) | `ken-mcp` mmaps the embedding model; CLI keeps the heap read | Accepted |
...
- **M2 — Lazy rerank model load** (commit fa5dc5e, [results/perf-startup-m2-results.md](results/perf-startup-m2-results.md)). New `internal/search/LazyReranker` defers `encoder.Load` + `LoadCacheFromFile` until the first `Rerank` call; sync.Once for single-shot under concurrent callers; thread-safe by construction. cmd/ken-mcp builds a closure that performs the 3-step block lazily. **−491 ms on ken-mcp startup when `KEN_MCP_RERANK=on`** — rerank-on startup is now indistinguishable from unset (~30 ms median, perf_startup_m2.sh).
- **M4 — Parallel `structural.Build`** (commit 34a52ae, [results/perf-startup-m4-results.md](results/perf-startup-m4-results.md)). Pass 1 of `internal/structural/Build` refactored to `runtime.NumCPU()` workers using the same per-file work pattern v0.8.7's chunk+embed parallelism shipped under ADR-030. Determinism preserved by writing per-file results into idx-aligned slice + merging in lexical order before Pass 2 builds lookup maps. **3.5× on jekyll (−1,127 ms), 4.5× on ken itself (−360 ms).**

**Killed milestones:**

- **M1 — Default Q8 rerank when present.** Closed without code change ([results/perf-startup-m1-results.md](results/perf-startup-m1-results.md)). Two reasons: (1) M2 already removed the rerank-load cost from the cold-start critical path, so M1's premise dissolved; (2) on Apple Silicon (the campaign's host platform), f32+NEON dominates int8 in both speed AND accuracy — the existing in-tree commentary in cmd/ken/main.go was right. Q8 remains available via `KEN_MCP_RERANK_QUANT=int8` for memory-constrained amd64/Linux deployments.
...
- **Config-key (invalidation).** A stable string over exactly the knobs that change the indexed *bytes* without changing the file *set*: format tag ⊕ mode ⊕ chunker ⊕ model fingerprint (dim + vocab + `model.safetensors` size — the model has no hash API) ⊕ enrichment on/off. Size caps (`KEN_MAX_FILE_BYTES`/`KEN_MAX_AVG_LINE_BYTES`) and ignore rules (`.gitignore`/`.kenignore`) are deliberately **not** keyed: flipping them adds/removes files from the walk (and a changed ignore file is itself indexed), so the manifest file set differs and the drift check catches them — keying them too would only add false-invalidation risk. Any key mismatch ⇒ full rebuild. (Mirrors the rerank cache's `scopeKey` gate.)
...
## ADR-040: Two-tier treesitter parse-timeout — disabled for reproducible CLI builds, bounded for the live server

**Status:** Accepted. **Date:** 2026-08-19. **Closes:** [#35](https://github.com/townsendmerino/ken/issues/35).

**Context.** Rebuilding the same corpus with the same config produced slightly different chunk counts across runs (postgres treesitter: 64,498 vs 64,506 = 0.012%; other rebuilds < ~0.1%). Root cause, code-confirmed: aikit's treesitter chunker parses under a **1s wall-clock timeout** (`parseTimeoutMicros`), and on `tree.ParseStoppedEarly()` routes the file to the **line** chunker — which emits a *different* chunk count than a full AST parse. A borderline file whose parse straddles ~1s completes on an idle machine but times out under load, so the fallback (and thus the chunk count) is **load-dependent**. v0.8.7's per-file parallelism supplies the CPU contention that tips borderline parses over the line; it is not itself the count-changing mechanism — the timeout→line fallback is. Cited files/line ranges never shifted (the drift is in files that don't affect ranked output), but `ken build-index` — the reproducible-embedded-index path SDK authors depend on (ADR-024) — was not byte-deterministic. This is the same class the enrichment budget already fixed (`KEN_ENRICH_FILE_BUDGET_MS` = 0/off in the CLI/library "to keep the build byte-deterministic"): a wall-clock budget left in the deterministic build path.

**Constraint that shapes the fix.** aikit exposes **only** a wall-clock parse timeout — no deterministic node/step budget (the same gotreesitter limitation ADR-016/#110 documented). So "provably deterministic" and "hang-protected against a runaway parse" are in tension: the only true runaway guard *is* the wall-clock timeout. And the `internal/search` layer **cannot** construct a configured treesitter instance — importing `aikit/chunk/treesitter` there inflates every binary ~26 MB (ADR-023 monolithic grammar embed), which is why treesitter is registered via side-effect import at the *binary main*, not the shared library (the "registry is the seam" rule).

**Decision.** Make the timeout policy a per-binary choice, set through the registry seam — no `internal/search` change, no per-build instances, no FSOptions threading.

- **aikit (`chunk/treesitter` v1.2.0, additive):** a functional option `WithParseTimeoutMicros(uint64)` on the `Chunker` (0 = **disable** the wall-clock timeout → parse outcome is a pure function of input bytes) plus a `ParseTimeout()` accessor. `New()` and the zero-value `Chunker` keep the 1s default, so every existing aikit caller — including the registry's `init()` instance — is byte-for-byte unchanged.
- **`cmd/ken` (CLI):** an `init()` re-registers `"treesitter"` with the timeout **disabled**. `ken build-index` / `ken index` index trusted local repos as a batch step and take reproducibility over the per-parse latency guard. Runaway-parse risk is covered by the 2 MiB `KEN_MAX_FILE_BYTES` walk cap, the deliberately-omitted bash grammar, and the #110-fixed grammars.
- **`cmd/ken-mcp` (server):** re-registers `"treesitter"` with the **1s** timeout (aikit's default, made explicit). The live multi-repo server keeps the latency guard so one pathological file can't stall a watch flush; it does not need byte-identical rebuilds (it rebuilds live, and snapshot reconcile keys on mtime+size, not chunk identity).

**Consequences / guardrails.**
- **Reproducibility contract:** `ken build-index` on the same corpus + config is now byte-identical across rebuilds regardless of machine load (verified: two ken-repo builds → identical bytes). Because the CLI never falls back on a timeout, borderline large files that used to *sometimes* degrade to line chunks now *always* get AST chunks — a small retrieval-quality gain on those files, and no more nondeterministic degradation.
- **Regression guards:** `cmd/ken` and `cmd/ken-mcp` each carry a test asserting their registered instance's `ParseTimeout()` tier; aikit tests the option + accessor. Reverting either registration fails a test loudly.
- **Residual (accepted):** the CLI build has no wall-clock runaway guard, so a genuinely pathological sub-2 MiB file could parse slowly (a batch delay, not a live-server stall; operator-interruptible). Given #110 fixed + bash omitted this is not observed on real corpora.
- **Library-direct callers** of `search.FromFS` / `BuildAndSerializeIndex` inherit whatever *their* binary registered (aikit's 1s default unless they register a disabled instance). The reproducible path is `ken build-index` — which the demos use; documented on `BuildAndSerializeIndex`.

**Rejected alternatives:** *Raise the const 1s → ~30s globally* (one-line, deterministic-in-practice since no real file parses near 30s, keeps a backstop) — rejected for the SDK-reproducibility case because it's only deterministic *in practice*, not provably, and the user chose the provable tier; kept on record as the cheaper fallback. *Per-build treesitter instance threaded through FSOptions* — blocked by the ADR-023 binary-size rule (can't import treesitter into `internal/search`) and would zero out `ken perf`'s registry-singleton `Stats()` read. *A deterministic node/step budget* — gotreesitter exposes none. *Flip aikit's default to disabled* — changes a calibrated shared-library default for all consumers; ken opts in at its binaries instead.

## ADR-041: Remote Streamable HTTP transport for ken-mcp (bearer auth, stateless, plain HTTP behind a proxy)

**Status:** Accepted. **Date:** 2026-08-19. **Closes:** [#15](https://github.com/townsendmerino/ken/issues/15).

**Context.** `ken-mcp` spoke JSON-RPC over stdio exclusively — it runs as a local subprocess of the agent, where the OS-user boundary is the auth boundary and there is no network exposure (the ADR-017 PII stance only had to defend against the local user). That is the right default, but it leaves out the *centralized dev box / staging server / team-shared instance* deployment: one ken-mcp feeding a fleet of agents, or remote-IDE setups where the agent isn't co-resident with the codebase. The MCP spec's forward-looking remote transport is **Streamable HTTP** (SSE is deprecated), and go-sdk v1.7.0 ships `NewStreamableHTTPHandler` returning a plain `http.Handler` — so the transport itself is a mounted handler, not a protocol to implement.

**Decision.** Add an opt-in Streamable HTTP transport, selected by `KEN_MCP_TRANSPORT=http` (default `stdio`, unchanged). The **same tool surface and the same `*sdk.Server`** serve both transports — HTTP is just a different I/O channel over shared process state (the ADR-013 LRU cache + Refresher). The security envelope every HTTP request passes through lives in `mcp/httptransport.go` (`NewHTTPHandler`), wired into the binary at `cmd/ken-mcp` (`transport.go` + the run/shutdown branch in `main.go`).

- **Bearer-token auth, mandatory.** `Authorization: Bearer <token>`, compared over SHA-256 digests (constant-time, no length/prefix timing leak). The token comes from `KEN_MCP_AUTH_TOKEN_FILE` (preferred — never enters the environment) or `KEN_MCP_AUTH_TOKEN`. **HTTP mode refuses to start without a token** (`HTTPConfig.Validate` → `ErrHTTPNoAuthToken` → `os.Exit(1)` with a fix message). No insecure-by-default, and no exception for a localhost bind (a localhost HTTP port is still reachable by every process on the box — a different threat model than stdio's single-parent-process boundary).
- **Stateless sessions** (`StreamableHTTPOptions{Stateless: true}`, the SEP-2567 sessionless direction). ken's tools are request/response; the only server→client traffic is the in-request progress heartbeat, which still reaches the client because it's emitted inside an incoming request's context. Stateless means no per-session server goroutine to leak and **no session affinity needed behind a load balancer** — which is exactly the centralized-deployment shape this transport targets.
- **PII hard-reject.** `KEN_DB_SAMPLE_ROWS>0` + `transport=http` exits non-zero at startup. A remote ken-mcp with row-sampling on is a network-accessible search index over sampled DB values; ADR-017's "operators with PII concerns shouldn't sample" was written for a local-only world. Fail loud rather than ship the foot-gun (the issue's own lean).
- **Per-client-IP token-bucket rate limiting** (`golang.org/x/time/rate`), `KEN_MCP_RATE_LIMIT` req/min (default 100; 0 disables). Outermost middleware, so an unauthenticated flood is throttled before it can DoS the auth comparison. Bounded limiter map (drop-all on overflow) caps memory against an IP spray.
- **No TLS in-process.** The deployment contract is **bearer-over-TLS terminated by a reverse proxy** in front of ken-mcp. `X-Forwarded-For` is deliberately *not* trusted for the rate-limit key (spoofable) — behind a proxy, per-IP degrades to a global cap, an acceptable DoS backstop.
- **Bounded graceful shutdown**, mirroring the stdio path: on signal, `http.Server.Shutdown` drains within `KEN_MCP_SHUTDOWN_GRACE`, a second signal force-quits. `ReadHeaderTimeout` guards slow-header DoS.

**Config via env vars, not flags.** `cmd/ken-mcp` has no `flag` surface — every knob is a `KEN_MCP_*` / `KEN_DB_*` env var (validated through `internal/envcfg`). The new controls follow suit (`KEN_MCP_TRANSPORT`, `KEN_MCP_ADDR`, `KEN_MCP_AUTH_TOKEN[_FILE]`, `KEN_MCP_RATE_LIMIT`) rather than introducing a parallel flag parser. Env is also *more* secret-safe than a `--auth-token` flag, which would appear in `ps`.

**Consequences / guardrails.**
- **stdio is untouched and remains the default** — the six `TestBinary_StdoutIsCleanJSONRPC_*` stdout-cleanliness audits still gate it. HTTP writes to the response body, not stdout, so the stdout contract is irrelevant there; a new HTTP test family covers it instead: middleware unit tests (`mcp/httptransport_test.go`), an in-process authed-session-over-HTTP test (`mcp/httptransport_integration_test.go`), and real-binary tests (`cmd/ken-mcp/http_transport_test.go`) for the three fail-loud guards plus a full authenticated `search` round-trip over a bound socket.
- **New security surface** exists only when `transport=http` is set; the default local deployment is unchanged.
- **`golang.org/x/time`** promoted from indirect to a direct dependency (already in the module graph; no new download).

**Rejected alternatives:** *`--transport` / `--auth-token-file` flags* — inconsistent with ken-mcp's env-only config surface, and flags leak tokens into `ps`. *Stateful sessions* — a per-session server goroutine to manage/leak and session affinity behind an LB, for no gain given ken's request/response tools. *Built-in TLS (`--tls-cert/--tls-key`)* — adds cert-loading/rotation surface; a TLS-terminating reverse proxy covers the deployment and is the operator's existing tool (deferred, re-openable). *SSE transport* — deprecated in the current spec. *No-auth on a localhost bind* — a localhost port is reachable by every local process; always-auth is the simpler invariant. *Separate `ken-mcp-remote` binary* — two artifacts / CI matrices / install snippets; one binary with a runtime mode is lighter. *mTLS / OAuth / per-tool authorization / request audit logging* — deferred to a v2 on real demand; bearer-over-TLS is sufficient for trusted-network v1.

---

## ADR-042: Result provenance for ken-mcp — a per-response index build id in JSON, full detail in `status`; markdown untouched

**Status:** Accepted (design; not yet implemented). **Date:** 2026-08-24. **Follows:** [`docs/internal/rag-thread-followups.md`](rag-thread-followups.md) item 4 (bench half shipped 2026-08-24).

**Context.** The r/Rag thread's provenance point was accepted for the benchmark harnesses: every bench result file now carries commit / chunker / mode / α pair / model digest / corpus revisions / `KEN_*` env (`bench/internal/provenance`, [`docs/BENCH.md` → "Result provenance"](../BENCH.md#result-provenance)). The commenter's second half was about *agent-facing* results — "why did ken return this yesterday" is currently unanswerable from a transcript. The follow-ups doc sketched an answer (an optional provenance line on every tool response behind `KEN_MCP_PROVENANCE=1`, default off to protect the token-economy story) and deferred the call to this ADR because the surface it touches — `FormatResults`, documented **1.0-stable, "changing it is a behavior change that needs an ADR"** — is the semble drop-in compatibility promise.

Two facts reshaped the decision away from that sketch:

- **The token argument is weaker than assumed.** A provenance line is ~25 tokens. Against the measured medians in `docs/BENCH.md` (semble bench, K=1 → 356–431 tokens; K=10 → 3,478–4,269) that is 0.6% at K=10 and ~6% at K=1. Real for the small-payload tools (`definition`, `references`), negligible for `search`. Not, on its own, a reason to refuse.
- **The redundancy argument is stronger than assumed.** Commit, chunker, mode, model digest and the α pair are **constant for the process lifetime**. Repeating constants on every response is noise, and the `status` tool already returns all of them (`status.Versions` — which gained a `Version` field with the bench work — plus `IndexInfo{Mode, Chunker, FileCount, ChunkCount, BuiltAt}` when called with `repo`). One `status` call per session already answers 95% of the question.

What `status` genuinely **cannot** answer retroactively is *which index generation served a particular response*. Watch mode republishes ~2 s after any file change (ADR-012), and an agent editing the repo mid-session moves the index underneath its own queries. That single fact varies per response and is not obtainable out-of-band.

**Decision.** Carry the varying part per response; leave the constant part where it already lives.

- **A per-response `index.build_id`** — 12 hex chars of `sha256(vcs.revision ‖ search.SnapshotConfigKey(...) ‖ generation)` — added to the **JSON** responses of the retrieval tools (`SearchResponse`, `FindRelatedResponse`). `SnapshotConfigKey` (ADR-039) already encodes mode, chunker, model fingerprint and the enrich/lazy/staged flags, so the id changes exactly when the index's identity changes; `generation` is the watch-mode republish counter. **Always present, no env var** — `mcp/json_responses.go` states the response structs are the 1.0-stable parse surface and that *adding* fields is safe, and an agent cannot build on a field that may or may not exist.
- **`status` reports the same `build_id`** alongside the full block, so `build_id` in a transcript resolves to a complete configuration via one call. That is the correlation mechanism: small token cost on the hot path, full detail on demand.
- **The markdown path is not touched at all.** `FormatResults` keeps its byte-for-byte semble parity, and the default output mode gains zero tokens. Agents wanting provenance opt in by asking for `output: "json"` — a choice they already make.
- **No `KEN_MCP_PROVENANCE` env var.** The sketch's opt-in flag was there to protect a token budget that turned out not to need protecting on the markdown path (untouched) or the JSON path (12 hex chars). An operator-set env var also can't be discovered by the agent, so a gated field is one an agent must treat as absent anyway.

**Consequences / guardrails.**
- **Purely additive to the JSON structs**, which is the documented-safe change class; markdown renderers are unchanged, so the `mcp` markdown golden tests keep passing untouched.
- **Semble drop-in compatibility is preserved literally** — semble has no `output: "json"`, so the entire change lands on a ken-only extension surface.
- **Cost is bounded by construction:** ~10 tokens on JSON responses, 0 on the default markdown path.
- **`build_id` is an identifier, not a description.** It is meaningless without a `status` call or the server's logs. That is the trade being made — one indirection to keep the hot path clean.
- **Not implemented yet.** This ADR records the decision; the code (build-id derivation in `mcp/bundle.go`, the two response fields, the `status` field, tests) is unscheduled. Items 1–3 of the follow-ups doc are ahead of it.

**Rejected alternatives:** *A provenance line on the markdown body (the follow-ups sketch)* — breaks the byte-level semble drop-in promise on the default path for information that is constant per process; the measured token cost (0.6–6%) was not the deciding factor, the redundancy and the 1.0-stable wire contract were. *`KEN_MCP_PROVENANCE=1`, default off* — an agent can't depend on an operator-set field, so the field ends up unused; and the budget it protects is 12 hex characters. *The full provenance block on every JSON response* — repeats per-process constants on every call for no gain over one `status` call. *Nothing at all ("`status` is enough")* — closest runner-up, and correct for everything except the index generation, which is exactly the part that varies and the part a mid-session-editing agent needs. *Persist a server-side response log* — answers the question far better, but adds a disk-writing audit surface (and its PII/retention questions) to a tool whose stance is to write nothing but the index; re-openable if operators ask for audit logging, which ADR-041 already deferred once.

## ADR-043: Dense-retriever seam — opt-in `FlatBinaryI8`, `HNSW` declined

**Status:** Accepted. **Date:** 2026-09-06. **Closes:** [roadmap A2](roadmap-2026-06.md#a2-wire-aikits-quantized-retrievers-into-kens-hybrid-path--done-measured-2026-09-06); resolves the "HNSW for the dense retriever" item in [`DESIGN.md` §10](../DESIGN.md#10-risk-register). **Full measurement:** [`dense-retriever-adoption-2026-09.md`](dense-retriever-adoption-2026-09.md).

**Context.** ken's semantic arm has always been `aikit/ann.Flat` — exhaustive f32 cosine, O(N) per query (`internal/search/index.go`, `ann.New(vecs)`). `DESIGN.md` §10 named the trigger to revisit this ("dense matrix size makes exact cosine the bottleneck on a real workload") back at aikit v0.2.0, when `HNSW` first shipped in aikit unwired. A 2026-08-15 pass (memory, not a committed doc) measured aikit-level scale/recall numbers for `FlatI8` alone and shelved it — not on recall, but because a full memory win needs an int8-throughout incremental-store refactor that was out of scope. This ADR closes the remaining half: measure all the wired-vs-unwired candidates through ken's actual pipeline (not just aikit's isolated `agree@k`) and decide.

**Measurement.** Two retained harnesses, `internal/search/retriever_eval_test.go` (real semble benchmark — 63 repos, 1251 queries, every retriever built from the same `ix.Vecs()`) and `retriever_scale_bench_test.go` (synthetic 13k→800k latency/build/memory ramp at ken's real dim, 256). Headline results: pipeline recall@10 is identical to 3 decimals across `Flat`/`FlatI8`/`FlatBinaryI8`/`HNSW` on the real benchmark (0.974 all / 0.971 NL / 0.995 symbol) — because `aikit/fuse.RRFWeighted` only consumes rank order, not raw cosine scores, an approximate retriever that gets the candidate set right costs the fused pipeline nothing. `FlatBinaryI8` (a two-stage binary-prefilter + int8-rerank retriever aikit added after the 2026-08-15 pass) is fastest at every N tested, repo scale included (2× at N=13k, 5.4× at N=800k), and 3.5× smaller. `HNSW`'s build time reproduces the prior 4m23s@200k anchor almost exactly (271.8s measured) and its memory is *larger* than `Flat`'s, not smaller (it keeps the full f32 vectors plus a graph).

**Decision.**

- **Introduce the small `Retriever`-style seam `DESIGN.md` §10 anticipated**, as the unexported `denseRetriever` interface (`Query(q []float32, k int) []ann.Hit`) in `internal/search/index.go`. Every candidate in `aikit/ann` already satisfies it — no new dependency, no interface aikit needs to add.
- **Wire `FlatBinaryI8` and `FlatI8` in as opt-in**, selected by `FSOptions.DenseRetriever` (`""` default / `"flat-i8"` / `"flat-binary-i8"`), read from `KEN_ANN` by `defaultFSOptions()` the same way `KEN_ENRICH` already is. `BuildIndex`, the public stable entry point, always builds `Flat` regardless of the env var — only the `FSOptions`-aware paths (`FromFSWithOptions`, the watch path) opt in, matching the existing `KEN_ENRICH` precedent exactly.
- **Do not adopt `HNSW`.** Its query is competitive from ~50k vecs on, but build cost is fundamentally incompatible with `ken index --watch`'s 2-second republish-on-edit contract, and it doesn't even win the memory argument. It solves neither of ken's two goals (latency, memory) the way `FlatBinaryI8` does without HNSW's downsides.
- **Do not flip the default yet.** The measured recall evidence covers repo/mid scale (real corpora, up to ~7k chunks per repo) and the measured latency/memory evidence covers kernel scale (up to 800k, synthetic vectors) — but no pass has measured recall on a real single corpus at kernel scale. That's the explicit trigger to flip: run `scripts/kernel_demo_bench.sh`'s ramp with `KEN_ANN=flat-binary-i8` and diff recall against the `Flat` baseline.

**Consequences / guardrails.**
- Zero behavior change for any existing caller — `denseRetriever`'s only implementor by default is still `*ann.Flat`, and `BuildIndex`'s signature and behavior are untouched.
- The ken-mcp watched/incremental path (`WatchedIndex.w.vecs`, retained f32 for append/compaction) does not get `FlatBinaryI8`'s full ~3.5× memory win when enabled there — it adds ~290 bytes/vec on top of the retained f32 vecs rather than replacing them, the same constraint the 2026-08-15 `FlatI8` pass already identified. The latency win is unconditional; the memory win on that specific path still needs the (unscheduled) int8-throughout incremental refactor.
- Regression-tested (`retriever_seam_test.go`): env-var parsing + fallback-on-typo, `BuildIndex` ignores the env default, each wired kind produces a working end-to-end hybrid index. The recall/latency/memory numbers themselves are pinned by the two bench-tagged harnesses, run on demand (`-tags=bench`), not on every `go test ./...`.

**Rejected alternatives:** *Adopt `HNSW` for kernel scale anyway, accepting the build-time cost as a one-time static-build tradeoff* — no static-build-only path exists in ken today (the CLI's `--no-watch` build still expects to run in reasonable CI time, and `ken-mcp` always watches), so this would need its own opt-in "frozen index, never rebuilt" mode to even be usable, and `FlatI8`/`FlatBinaryI8` already win without that extra surface. *Flip the default to `FlatBinaryI8` now* — the repo/mid-scale evidence is strong enough to justify it there, but doing so unconditionally would silently change behavior at kernel scale before the recall gap above is closed; shipping the knob first keeps that a deliberate, reversible follow-up (and the follow-up below shows this caution was warranted). *Compose `FlatBinary` (not `FlatBinaryI8`) ken-side as the two-stage shortlist* — aikit already ships the combined type; building it again in ken would just be a second copy of `binaryPrefilter` to keep in sync.

**Update (2026-09-06, same day — kernel-scale spot-check closes the gap this ADR left open):** the "kernel-scale (500k+) real-corpus recall spot-check" this ADR named as the trigger to flip any default was run against a real 584,019-chunk sparse Linux kernel checkout (retained as `internal/search/kernel_scale_spotcheck_test.go`, opt-in via `KEN_KERNEL_CORPUS`). Result: `FlatI8` holds at real kernel scale (agree@10 vs exact `Flat` = 1.00 on realistic domain queries, 0.98 on a self-retrieval near-duplicate stress test) — but `FlatBinaryI8` does **not**: its agreement drops to 0.96 on both query sets, a real, quantified gap the repo-scale real-qrel evidence and the synthetic-vector scale ramp both missed (real code's semantic clustering at real scale is exactly what a binary Hamming prefilter is more exposed to than either small real corpora or synthetic near-orthogonal vectors). This revises the per-regime recommendation: **`FlatI8` is now the recommended choice at real kernel scale**, not `FlatBinaryI8` — the original "projected, not yet confirmed" language above undersold the risk that the projection turned out to be wrong specifically for `FlatBinaryI8`. `FlatBinaryI8` remains the right choice at repo/mid scale, where the zero-cost evidence is real qrels, not raw agreement. Full writeup: [`dense-retriever-adoption-2026-09.md`](dense-retriever-adoption-2026-09.md)'s "Kernel-scale spot-check" section. Separately, the first spot-check attempt (same corpus, Arm B enrichment left on) hung past a 30-minute timeout inside an unbounded `gotreesitter` parse on some file under the real driver tree — investigated the next day as its own issue; see ADR-044.

## ADR-044: CLI default enrichment parse budget (`build-index` excepted)

**Status:** Accepted. **Date:** 2026-09-07. **Follows:** the kernel-scale spot-check in [ADR-043](#adr-043-dense-retriever-seam--optin-flatbinaryi8-hnsw-declined)'s update note. **Full writeup:** [`gotreesitter-c-large-init-array-slowdown.md`](gotreesitter-c-large-init-array-slowdown.md).

**Context.** `ken-mcp` has always applied a 500ms default per-file structural-enrichment parse budget (`cmd/ken-mcp/enrich_budget.go`) — the CLI (`ken index`/`search`/`bench`/`perf`) and library callers of `search.FromFS`/`FromPath` have never had one (`KEN_ENRICH_FILE_BUDGET_MS` defaults to `""` → 0 → unbounded, deliberately, per `internal/structural/index.go`'s determinism note). This was a documented, accepted risk for two known single-file pathologies (C#'s parked grammar, occasional Swift 2-6s hangs). Investigating a hang hit while spot-checking the dense-retriever work at kernel scale (ADR-043) found the same failure *class* on plain `.c`/`.h` files: 504 of 37,775 real files (1.33%) in a sparse Linux kernel checkout exceeded a 3s parse budget — large designated-initializer arrays common in driver code triggering a `gotreesitter` `c`-grammar slowdown (filed upstream, [gotreesitter#1100](https://github.com/odvcencio/gotreesitter/issues/1100), open). That's a materially different risk profile from "one rare pathological file": C is one of ken's 15 core enrichment languages, and the breadth (many files, many unrelated subsystems, one real corpus) means any of the other 14 enrichment grammars plausibly has a similar undiscovered defect.

**Decision.** Give `ken index`/`search`/`bench`/`perf` a default, generous, non-zero enrichment budget — protecting all 15 enrichment languages uniformly, since the budget mechanism (`extractGuarded`/`envParseBudgetMicros`) is grammar-agnostic — while leaving `ken build-index` and direct `FSOptions{}` callers (`BuildAndSerializeIndex`, used by `mcp.Run`'s embedded-corpus build) unbounded, unchanged.

- **`cmd/ken/enrich_budget.go`**, mirroring `cmd/ken-mcp`'s own pattern: `setupEnrichBudget()` defaults `KEN_ENRICH_FILE_BUDGET_MS` to **2000ms** when unset (4× ken-mcp's 500ms — these commands tolerate more per-file latency than an interactive server, but shouldn't be unbounded) and routes budget-exceeded skips to stderr. Called from `main()` for `index`/`search`/`bench`/`perf` only.
- **Not applied to `build-index`.** Its whole contract is byte-identical, machine-load-independent output ([ADR-040](#adr-040-two-tier-treesitter-parse-timeout--disabled-for-reproducible-cli-builds-bounded-for-the-live-server)) — a wall-clock budget is exactly the load-dependence that ADR closed for the chunker, and reopening it here for enrichment would undo that guarantee for the one command that explicitly promises it.
- **Not a library-level (`internal/search`) change.** `defaultFSOptions()` is untouched — the new default lives at the `cmd/ken` binary layer only, exactly where ken-mcp's already lives, for the same reason: the underlying `gotreesitter` parser pools are cached process-globally per grammar name (`internal/structural`'s `parserPools sync.Map`), not per-caller, so a library-level default would leak into *any* other caller sharing the same test/embedder process (e.g. `internal/search`'s own `go test` binary, which also runs `BuildAndSerializeIndex`/golden/determinism tests) — reintroducing the exact cross-call contamination ADR-040 and the parse-budget flakiness fix (`5c185fe`) already had to clean up once. Setting the default once, at each *binary's* `main()`, before any parse happens, is the only safe place given that architecture.

**Why this is an acceptable determinism trade (and why `build-index` still isn't included).** A budget-triggered skip changes one chunk's *text* (whether the enrichment label is prepended) — a narrower form of non-determinism than what ADR-040 fixed (chunk *count*, which cascades into every downstream index invariant: vecs/chunks alignment, serialization, `ann.Flat` row identity). `index`/`search`/`bench`/`perf` make no byte-identical promise today, so accepting machine-load-dependent enrichment content there costs nothing that wasn't already true of watch-mode's live republishing. `build-index` is the one command that promises reproducibility, so it keeps the stronger (unbounded) guarantee and the residual risk stays explicitly documented, same as Swift's in `DESIGN.md` §10.

**Consequences / guardrails.**
- Regression-tested: `cmd/ken/enrich_budget_test.go` asserts the default applies only when unset and never overwrites an operator's explicit value.
- A pathological file now reads as "enrichment skipped for this file" on stderr instead of looking like a hang — matching the spirit of v1.5.1's C# fix title ("fix an apparent hang").
- `ken build-index` and `mcp.Run` embedders remain exposed to this failure class by design; re-open this ADR if that trade stops being acceptable (e.g. if a deterministic node/iteration cap lands upstream — see the feature request in the tracking doc — which would let `build-index` get protection too, without the wall-clock trade).

**Rejected alternatives:** *Change `internal/structural`'s or `internal/search`'s own default* — breaks the golden/parity/determinism test suite's implicit assumption of an unbounded budget the moment any other test in the same process calls a path that sets it (the process-global parser-pool cache makes this a real, not theoretical, risk). *Apply the budget to `build-index` too, accepting the load-dependence* — undoes ADR-040 for the one command whose entire purpose is the guarantee ADR-040 provides; if `build-index` needs protection, it needs a deterministic (not wall-clock) mechanism, tracked as a follow-up pending upstream. *Lower the default to match ken-mcp's 500ms* — appropriate for an interactive server's latency budget, needlessly tight for a batch CLI build where a few extra seconds on a handful of files is a fair trade against false-positive skips on merely-large-but-fine files.

---

## ADR-045: `ken-mcp` mmaps the embedding model; CLI keeps the heap read

**Status:** Accepted. **Date:** 2026-09-10. **Related:** reopens the trade-off [ADR-004](#adr-004-hand-rolled-safetensors-reader) made in the opposite direction; the v0.6.0 embedded-corpus feature (`embed.LoadFromFS`, `Options.ModelFS`, cross-referenced above) is why that mmap got dropped in the first place.

**Context.** ADR-004 chose to mmap `model.safetensors` specifically to avoid a 64 MB heap read. That intent didn't survive v0.6.0: making `fs.FS` the canonical model-loading surface (needed so `mcp.Run` embedders can bake a model in via `//go:embed`, which has no real file path to mmap) meant every loader — `embed.LoadFromFS`, and by extension every ken call site that resolves a model from a directory (`internal/search/index.go`'s live-build path, and `cmd/ken-mcp`'s pre-built-index and M1-snapshot loads) — went through `fs.ReadFile`, a full heap read, even for callers that do have a real directory. aikit later added `embed.LoadMmap(dir string)` (a real-path-only sibling of `LoadFromFS`, measured 5.8× less peak heap / 17% slower cold start on the embedded-corpus example) specifically to give that back to callers who want it — but nothing in ken called it. Reported trigger: the user noticed `ken-mcp` resident at ~1.6 GB and asked for ways to cut it. `ken-mcp` is the case ADR-004's original worry applies to hardest: it's long-lived (pays the resident cost indefinitely, not just at cold start), and its per-process LRU (`KEN_MCP_CACHE_SIZE`, default 16) can hold several repos' `WatchedIndex`es at once, each of which independently heap-read its own 64 MB copy of the model — no in-process sharing existed. A read-only mmap doesn't just avoid the heap copy; the kernel shares the same physical pages across every `mmap()` of the same file, so N cached repos stop paying N× once mmap is in use.

**Decision.** Add `FSOptions.MmapModel bool` (`internal/search/index.go`) and a single dispatch point, `search.LoadModelDir(modelDir string, mmap bool) (*embed.StaticModel, error)`, that every model-loading call site now goes through: `false` → `embed.LoadFromFS` (unchanged), `true` → `embed.LoadMmap`. `ken-mcp` sets it from a new env var, **`KEN_MCP_MMAP_MODEL`** (default `true`) — applied uniformly across its three load sites (`internal/search/index.go`'s live-build path via `FSOptions`, `cmd/ken-mcp/main.go`'s pre-built-index load, `cmd/ken-mcp/snapshot.go`'s M1 snapshot load), all of which already threaded an `FSOptions` value through to this point. The CLI (`ken index`/`search`/`build-index`) and the `mcp.Run` embedded-corpus path are **unchanged** (`MmapModel` zero value is `false`): short-lived processes where cold-start latency dominates over resident footprint, and `mcp.Run`'s `ModelFS` case has no real directory to mmap regardless.

**Measured.** `go run ./tools/peakrss -- <harness> {heap,mmap} . ~/.ken/model`, building a hybrid index over ken's own repo (2,752 chunks): peak RSS 306,240 KB → 250,448 KB (**−18%, ~56 MB**), wall time not worse (page cache was already warm from the prior run). Scales with however many distinct repos a given `ken-mcp` process has cached — the more repos, the bigger the win, since the model-copy cost was previously per-repo.

**Consequences / guardrails.**
- `internal/search/model_mmap_test.go` pins the wiring two ways: `LoadModelDir(mmap=true)` and `LoadModelDir(mmap=false)` produce bit-identical embeddings for the same input (mmap only changes where the bytes come from, never their values), and `FSOptions{MmapModel: true}` reaches the live-build path end-to-end (`FromFSWithOptions` → a queryable semantic index).
- `KEN_MCP_MMAP_MODEL=0` is the escape hatch if a deployment's model directory sits on a filesystem where mmap misbehaves (e.g. certain network mounts) — falls back to the unchanged heap-read behavior. `embed.LoadMmap` already degrades to a heap read on non-unix targets internally, so Windows needs no special casing here.
- Model *identity* (the fingerprint used for the M1 snapshot config-key, `ModelFingerprint`/`ModelFingerprintFromDir`) is based on the file itself, not the loading method — `MmapModel` flipping does not change a snapshot's config-key or force a rebuild.

**Rejected alternatives:** *Add a process-wide model cache keyed by `(modelDir)` instead of mmap.* Would also dedupe the N-repos-N-copies cost, but is strictly worse than mmap alone: it still pays one full heap copy (mmap pays none), adds a new shared-mutable-state lifetime to reason about (when is it safe to evict / does an evicted repo's model outlive the repo), and doesn't help the single-repo case at all, whereas mmap helps both. *Make `MmapModel` default `true` everywhere, including the CLI.* Rejected per aikit's own measured trade (footprint win, latency cost) and its doc comment's explicit guidance — a one-shot `ken search` pays the 17%-slower load on every invocation for a peak-RSS win that a short-lived process never benefits from holding onto.
