...
Toolchain: `go.mod` pins `go 1.26.6` (the go directive doubles as the toolchain floor; no separate `toolchain` line). With `GOTOOLCHAIN=auto` (default) an older system Go auto-downloads 1.26.6. (Bumped 1.26.3 → 1.26.4 for the go1.26.4 stdlib CVE fixes `govulncheck` flagged, then 1.26.4 → 1.26.5 for GO-2026-5856, the crypto/tls Encrypted-Client-Hello privacy leak the govulncheck gate caught as reachable via the MySQL TLS + model-fetch HTTPS paths, then 1.26.5 → 1.26.6 for the 2026-08-13 Go security release — fixes across the `go` command + `crypto/tls`, `net/http`, `net/url`, `encoding/asn1`, `encoding/xml`, `html/template`, several reachable via the same TLS + HTTPS paths; taken as toolchain security-currency, and the `govulncheck` gate stays clean — see `.github/workflows/govulncheck.yml`.) **A `1.26.6 → 1.27.0` bump was attempted + reverted 2026-08-19: no golangci-lint/staticcheck release yet supports go 1.27** — staticcheck's `buildir` crashes analyzing go 1.27's Linux `internal/poll` (`unexpected expr: *ast.KeyValueExpr`), which fails the required Linux lint CI job (it passes on macOS, whose `internal/poll` uses kqueue, so a local `make check` won't catch it). Building golangci-lint with go 1.27 doesn't help — the incompatibility is in staticcheck's vendored IR builder. Re-attempt once a golangci-lint release bundles a go1.27-ready staticcheck. The 1.22-era `go fix ./...` modernizers from that attempt (range-over-int, `min`/`max` builtins) were kept — they're valid at 1.26.6. All CI reads the version via `go-version-file: go.mod`, so this one directive is the single toolchain source of truth. Primary deps: `github.com/townsendmerino/aikit` (+ `aikit/chunk/treesitter` submodule) — the extracted algorithm packages (ADR-034), pin tagged versions and re-run the parity/recall checks on bump; `github.com/odvcencio/gotreesitter` (treesitter grammars), `github.com/go-git/go-git/v5` (ken-mcp clone), `github.com/modelcontextprotocol/go-sdk` (ken-mcp), `github.com/fsnotify/fsnotify` (watch mode), `golang.org/x/sync` (singleflight), plus the DB drivers (`modernc.org/sqlite`, `github.com/jackc/pgx/v5`, `github.com/go-sql-driver/mysql`). After a `go`/`toolchain`/aikit bump, run `go mod tidy`.
