...
Toolchain: `go.mod` pins `go 1.27.0` (the go directive doubles as the toolchain floor; no separate `toolchain` line). With `GOTOOLCHAIN=auto` (default) an older system Go auto-downloads 1.27.0. (Bumped 1.26.3 → 1.26.4 for the go1.26.4 stdlib CVE fixes `govulncheck` flagged, then 1.26.4 → 1.26.5 for GO-2026-5856, the crypto/tls Encrypted-Client-Hello privacy leak the govulncheck gate caught as reachable via the MySQL TLS + model-fetch HTTPS paths, then 1.26.5 → 1.26.6 for the 2026-08-13 Go security release — fixes across the `go` command + `crypto/tls`, `net/http`, `net/url`, `encoding/asn1`, `encoding/xml`, `html/template`, several reachable via the same TLS + HTTPS paths; taken as toolchain security-currency, and the `govulncheck` gate stays clean — see `.github/workflows/govulncheck.yml`.) The **1.26.6 → 1.27.0** bump (2026-08-20, toolchain currency, aligning with aikit's `go 1.27.0`) was blocked for a day: it fails the required Linux lint job on any golangci-lint whose bundled **staticcheck** predates go 1.27 (staticcheck's `buildir` crashes on go 1.27's Linux `internal/poll` — `unexpected expr: *ast.KeyValueExpr`; note this passes on macOS/kqueue, so a local `make check` won't catch it). **golangci-lint v2.13.0 (2026-08-19) added go 1.27 support**, and its prebuilt binaries are go1.27-built — so CI's `golangci-lint-action` pins `version: v2.13.0` in the default binary mode, and the Makefile pin matches. `go fix ./...` applied the 1.22-era modernizers (range-over-int, `min`/`max` builtins). Locally, install golangci-lint ≥ v2.13.0 built with go 1.27 (`go install github.com/golangci/golangci-lint/v2/cmd/golangci-lint@v2.13.0` under Go 1.27). All CI reads the version via `go-version-file: go.mod`, so this one directive is the single toolchain source of truth. Primary deps: `github.com/townsendmerino/aikit` (+ `aikit/chunk/treesitter` submodule) — the extracted algorithm packages (ADR-034), pin tagged versions and re-run the parity/recall checks on bump; `github.com/odvcencio/gotreesitter` (treesitter grammars), `github.com/go-git/go-git/v5` (ken-mcp clone), `github.com/modelcontextprotocol/go-sdk` (ken-mcp), `github.com/fsnotify/fsnotify` (watch mode), `golang.org/x/sync` (singleflight), plus the DB drivers (`modernc.org/sqlite`, `github.com/jackc/pgx/v5`, `github.com/go-sql-driver/mysql`). After a `go`/`toolchain`/aikit bump, run `go mod tidy`.
...
**Arm B enrichment (ADR-035, Stage 8 close):** for every chunk produced from a file whose extension has a registered gotreesitter extractor (`.py`, `.go`, `.ts/.tsx`, `.js/.jsx/.mjs/.cjs`, `.java`, `.rs`, `.cpp/.cc/.cxx/.hpp/.hh/.hxx`, `.c/.h`, `.php`, `.rb`, `.kt/.kts`, `.dart`, `.swift`, `.scala/.sc`, `.ex/.exs` — 15 languages; `.cs` is **parked** as of 2026-08-24, see DESIGN.md §10), the indexer prepends a deterministic per-file label line `# func: NAME | calls: A, B | raises: X\n` before BM25 tokenization and embedding. Pure-Go, no extra model. Default-on; opt out with `KEN_ENRICH=off` or `FSOptions.DisableEnrichment=true`. **Lazy/async mode (cold-start M2, `FSOptions.LazyEnrichment` / `KEN_MCP_LAZY_ENRICH=1`, opt-in):** the cold build serves a raw index for a fast first query, then a background pass (`WatchedIndex.enrichCorpusInBackground`, fresh-slice atomic republish) enriches + republishes; ~2–3× faster first-servable on PHP, results well-formed (label-only, additive) but lower-ranked until it lands; a snapshot load is already enriched so it only bites a true-cold build. A per-file wall-clock parse budget (`KEN_ENRICH_FILE_BUDGET_MS`, default `500` in ken-mcp, `0`/off in the CLI/library to keep the build byte-deterministic) skips + logs + counts any file whose gotreesitter parse exceeds it (a timeout surfaces as `ParseStopReason==ParseStopTimeout`, which `extractGuarded` already rejects), guarding against individually pathological template-like files the 64 KiB size cap misses (`structural.ParseBudgetSkips()` / `structural.SetParseBudgetLogf`). In-process bench shows **+0.0208 NDCG@10 hybrid on csn-python-nl-stripped (N=500)** and **+0.0321 on CoSQA dev** — reproducing the validated Gate-1 numbers within 0.002 on the production code path. Files with no registered extractor pass through unchanged. Single source of truth: `structural.EnrichFromFileStruct` (no-index per-file path) and `(Index).Enrich` (with optional cross-file callers) both delegate to `enrichCore`.
...
- **`internal/repo`** (ken) — gitignore-respecting filesystem walk; prunes `.git`, skips binary (NUL sniff), oversized (`KEN_MAX_FILE_BYTES`, default 2 MiB), and minified files (`KEN_MAX_AVG_LINE_BYTES` avg-line-length heuristic, ADR-038 memory campaign M3); rejects repos over the `KEN_MAX_FILES` admission cap (default 1M, `ErrTooManyFiles`) rather than risking OOM on hostile input. **Opt-in generated-code skip** (`KEN_SKIP_GENERATED`, default off): when on, files whose header carries a machine-generated marker (`@generated`, or a `Code generated … DO NOT EDIT` banner — `looksGenerated`) are skipped at index time. Distinct from the line-shape minified sniff — this catches normally-shaped-but-generated source (protobuf stubs, client-go listers, mocks). Off by default because some corpora legitimately want their generated surface searchable; honored by both the initial `WalkFS` and the watch-path `Matcher`. Also honors a **`.kenignore` / `.sembleignore`** ignore family (ADR-038) as an independent union with `.gitignore` (two-family evaluation, no cross-file re-includes). The matcher is a deliberate common-subset of gitignore; full pathspec parity is a later dependency swap (`docs/DESIGN.md` §1).
- **`aikit/chunk`** — `Chunk` is the stable unit every stage depends on. Three chunkers land behind a single `Chunker` interface (registered via `database/sql`-style blank imports to avoid an import cycle): `line` (universal 50-line/5-overlap fallback), `regex` (default; per-language rules for Python/Go/TypeScript/Java/Rust + line fallback), and `treesitter` (opt-in as of v0.2.0; pure-Go tree-sitter via `gotreesitter` running cAST split-then-merge — `aikit/chunk/treesitter`, ADR-010/011 in [`docs/internal/DECISIONS.md`](docs/internal/DECISIONS.md)). The byte-fidelity invariant (concat of `Chunk.Text` reproduces source) holds across all three. **Public package as of ADR-032** — the `Chunker` interface is the 1.0-stable surface; the concrete chunkers (especially treesitter, gotreesitter-backed) are best-effort. **Two-tier treesitter parse timeout (ADR-040, #35):** the treesitter chunker's per-parse **wall-clock** timeout made chunk *counts* load-dependent (a borderline file times out → line-chunker fallback → different count → non-reproducible `ken build-index`). `aikit/chunk/treesitter` v1.2.0 adds `WithParseTimeoutMicros` (0 = disable → deterministic parse); `cmd/ken` registers the chunker with it **disabled** for byte-identical CLI builds, while `cmd/ken-mcp` registers the bounded **1 s** default (latency guard for the live server). Set at each binary's `main` via the registry — `internal/search` can't import treesitter (ADR-023 binary-size seam). Distinct from the Arm B enrichment budget above (that governs the structural *label*, not chunk count).
...
- **`aikit/ann`** (`flat.go`) — flat brute-force cosine retriever over the dense matrix. Scoring uses the SIMD dot kernel (`linalg.Dot`, **float32-precision** as of aikit 1.4 — near-exact, reorders only sub-1e-6 ties vs a float64 scan; recall@10 re-verified unchanged). `Flat` is the default and fine at repo scale. **`FSOptions.DenseRetriever` / `KEN_ANN=flat-i8|flat-binary-i8`** (opt-in, default unchanged) swaps it for `ann.FlatI8` or `ann.FlatBinaryI8` — measured zero end-to-end recall cost and a latency/memory win at every scale tested, repo scale included; see [ADR-043](docs/internal/DECISIONS.md#adr-043-dense-retriever-seam--optin-flatbinaryi8-hnsw-declined) and [`docs/internal/dense-retriever-adoption-2026-09.md`](docs/internal/dense-retriever-adoption-2026-09.md). `HNSW` was evaluated and declined — sub-linear query but build cost (271.8s@200k measured) breaks `ken index --watch`'s 2s republish contract, and it costs more memory than `Flat`, not less.
...
- `KEN_MCP_TRANSPORT` — `stdio` (default) / `http` (ADR-041, #15). `stdio` is the local-subprocess default (OS-user boundary = auth boundary), unchanged. `http` opts into the network-exposed **Streamable HTTP** transport (go-sdk `NewStreamableHTTPHandler`, stateless per SEP-2567) for centralized/staging/team-shared deployments — same tool surface, same `*sdk.Server`, just a different I/O channel. Requires the auth + PII guards below; **fails loud at startup** on any insecure combination. Security envelope lives in `mcp/httptransport.go` (`NewHTTPHandler`); wiring in `cmd/ken-mcp/transport.go`.
- `KEN_MCP_ADDR` — HTTP bind address (default `:8080`; only used when `KEN_MCP_TRANSPORT=http`). No TLS in-process — front ken-mcp with a TLS-terminating reverse proxy (bearer-over-TLS is the deployment contract).
- `KEN_MCP_AUTH_TOKEN` / `KEN_MCP_AUTH_TOKEN_FILE` — bearer token for HTTP mode (`_FILE` preferred: the secret never enters the environment). **Mandatory for `http` — ken-mcp refuses to start without it** (no insecure default; no localhost exception). Compared over SHA-256 digests (constant-time, no length leak). Ignored in stdio mode.
- `KEN_MCP_RATE_LIMIT` — requests/min per client IP for HTTP mode (default `100`; `0` disables). Per-IP token bucket (`golang.org/x/time/rate`), outermost so an unauthenticated flood can't DoS the auth check. Behind a proxy this degrades to a global cap (`X-Forwarded-For` is not trusted — spoofable). NOTE: `KEN_DB_SAMPLE_ROWS>0` is **hard-rejected** with `KEN_MCP_TRANSPORT=http` (sampled DB values would be network-searchable — extends the ADR-017 PII stance).
...
- `KEN_MCP_LAZY_ENRICH` / `KEN_MCP_STAGED` / `KEN_MCP_EMBED_CACHE` — `1`/`0` (default `0`, all opt-in). Cold-start M2/M4/M3: **M2** defers the enrichment parse (serve raw hybrid, enrich in background); **M4** staged defers *both* parse + embed (serve BM25 lexical instantly ~4× faster first query, then upgrade to hybrid; responses carry `"semantic":"warming"`; supersedes M2 when both set); **M3** is a persistent `sha256(chunk)→vector` cache at `.ken/embed.db` so a full rebuild re-embeds only new text (`KEN_MCP_EMBED_CACHE_MAX` caps it). All only affect a true-cold build; a snapshot load (M1) is already full. Only affect the WatchedIndex (server) path.
...
- `KEN_MCP_SHUTDOWN_GRACE` — bounded drain window after SIGINT/SIGTERM (Go duration, default `5s`); in-flight tool calls drain within it (exit 0), then main force-exits with status 1. A second signal force-quits immediately with 128+signal (SIGINT → 130, SIGTERM → 143). Shutdown threads the signal context into the index build so a large in-flight build honors cancellation (Task B fix; `NewWatchedIndexWithContext`).
- `KEN_MCP_ALLOWED_REPO_ROOTS` — optional local-path confinement (unset ⇒ off). OS-path-list-separated absolute roots; an **agent-supplied** local-path `repo` argument must resolve (symlinks resolved) under one of them, else the tool errors. The local-path analogue of the `KEN_ALLOW_PRIVATE_CLONE_TARGETS` clone SSRF guard, for the long-lived-server/untrusted-agent deployment. `KEN_MCP_DEFAULT_REPO` is exempt. Enforced in `resolveRepo` (`mcp/confine.go`), the choke point every repo-taking tool shares. **When unset, ken-mcp logs a loud WARN-level startup security warning** (`mcp.RepoConfinementWarning`) — an unconfined long-lived server can be steered by a prompt-injected agent into reading arbitrary local files; setting the var confines it and silences the warning.

All env vars are validated at startup. Invalid values (typoed enums like `KEN_MCP_MODE=hybryd`, non-integer `KEN_MCP_CACHE_SIZE=of`, `KEN_MCP_MODEL_DIR` pointing at a non-existent path) log a stderr warning and fall back to the documented default — the silent-typo failure mode (where `Atoi("of")` returned 0 and disabled the cache) is gone. Enum match is case-sensitive: `Hybrid` warns and falls back to `hybrid`. Helpers live in `internal/envcfg` (`EnvInt` / `EnvEnum` / `EnvPath` / `EnvPathOrURL`), extracted from `cmd/ken-mcp` so they're unit-tested as a standalone package and reusable by other server binaries.
