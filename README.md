...
| `KEN_MCP_ALLOWED_REPO_ROOTS` | (unset) | Confine **agent-supplied** local-path `repo` arguments to these roots (OS-path-list-separated, like `PATH`). Unset = no confinement (any local path). The local-path analogue of the clone SSRF guard, for a long-lived server where an untrusted agent supplies `repo`: it stops the agent pointing ken at `/etc`, `~/.ssh`, etc. `KEN_MCP_DEFAULT_REPO` is exempt (the operator vouched for it). Symlinks are resolved before the check. |
