...
| `KEN_MCP_SHUTDOWN_GRACE` | `5s` | After a SIGINT/SIGTERM, how long to let in-flight tool calls drain before forcing exit (any Go duration). A clean drain exits 0; a forced exit is status 1, and a second signal force-quits immediately with 128+signal (130 / 143). |
...
| `KEN_ENRICH_FILE_BUDGET_MS` | `500` (ken-mcp); `2000` (`ken index`/`search`/`bench`/`perf`); `0` (`ken build-index`, library) | Per-file wall-clock cap on the Arm B enrichment / structural tree-sitter parse. A file whose parse exceeds it is skipped (indexed without its structural label — results are unaffected, just unenriched), logged, and counted. Guards against individually pathological template-like files that the size cap misses (real example: large designated-initializer arrays in driver-style C, [gotreesitter#1100](https://github.com/odvcencio/gotreesitter/issues/1100)). `0` disables. Off by default only for `ken build-index` and direct library callers (`BuildAndSerializeIndex`), to keep that specific build byte-deterministic. |
