...
- **`mcp/db` opt-in package** — SDK authors using `mcp.Run` can now wire Tier 2 DB support via `mcpdb.Setup(ctx, mcpdb.Config{DSN: ..., EnableListen: true, ReindexInterval: 5 * time.Minute, ...})`, which returns a `mcp.ReindexFunc` to pass as `opts.Reindex`. Activates schema introspection (validates DSN; captures freshness) + LISTEN/NOTIFY (Part 1) + interval reindex + `reindex_db` MCP tool (Part 2) in one helper. **v0.6.0 binary-size contract preserved** — SDK authors who don't import `mcp/db` get a binary identical in dep tree to v0.7.2's `mcp.Run` use case. DB driver tree (pgx + sqlite + mysql + internal/db) is opt-in by import. Closes Part 3 of [#12](https://github.com/townsendmerino/ken/issues/12). ([ADR-020 Part 3](docs/DECISIONS.md#part-3-opt-in-mcpdb-package-preserving-v060-binary-size-contract-v080-part-3))
...
- **`mcp.Options.Reindex`** — new `ReindexFunc` field on the embedded-corpus `Options` struct. Optional (nil = no DB tool). `mcp.Run`'s `newServerForIndex` registers the `reindex_db` MCP tool conditionally on `opts.Reindex != nil`, matching `NewServer`'s Part 2 conditional-registration pattern. SDK authors who don't import `mcp/db` get a two-tool surface (search + find_related — identical to v0.6.0 / v0.7.x).
...
- **Chunk visibility into `mcp.Run`'s static `*search.Index` is deferred to v0.9.0.** In v0.8.0 Part 3, `mcp/db.Setup` runs introspection on each refresh (validates the DSN, captures freshness, fires LISTEN handlers, exercises the interval-ticker path) and the `reindex_db` tool returns the standard `Reindexed in Nms.` response — but the chunks `IndexSchema` produces are NOT yet unioned into the embedded `*search.Index` that `mcp.Run` serves. Chunk integration requires a `SetExtraChunks`-style mechanism on the static `*Index` that v0.8.0 doesn't ship; `cmd/ken-mcp` uses `*WatchedIndex.SetExtraChunks` which doesn't generalize to `mcp.Run`'s `fs.FS`-rooted static Index. ADR-020 Part 3's Consequences section documents the deferral with the rationale and the v0.9.0 follow-up plan. The Setup's swap callback logs the deferral message at runtime via the operator's `LogWriter` so SDK authors / operators see the limitation in their stderr (not just buried in docs).
- **v0.6.0 binary-size contract preserved.** SDK authors building docs-only embedded-corpus binaries continue to get the v0.6.0 small-binary dep posture — `mcp` package's import set is unchanged across v0.6.0 → v0.8.0. The contract is now enforced by the `TestBinary_MCPPackageStaysDBFree` test (CI grep step via `go list -deps`); a future commit accidentally adding a DB import to `mcp/` fails CI before merging.
