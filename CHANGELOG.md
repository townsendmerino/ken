...
- **Stdout-cleanliness invariants unchanged.** All seven stdout-clean variants (stock / Postgres / SQLite / MySQL / LISTEN/NOTIFY / reindex_db / MariaDB) continue to pass byte-identically. `mcp.Run` reads from the supplied `fs.FS`; no new stdout-clean variant is required because no new daemon-path component was added.
