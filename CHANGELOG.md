...
- **Stdout-cleanliness invariants unchanged.** All seven stdout-clean variants (stock / Postgres / SQLite / MySQL / LISTEN/NOTIFY / reindex_db / MariaDB) continue to pass byte-identically. The stock + SQLite variants run on every checkout's `go test ./cmd/ken-mcp/`; the other five require service containers via the `test-db-integration` job and are CI-only — `KEN_DB_TEST_DSN` / `KEN_DB_MYSQL_TEST_DSN` / `KEN_DB_MARIADB_TEST_DSN` gate them locally. `mcp.Run` reads from the supplied `fs.FS`; no new stdout-clean variant is required because no new daemon-path component was added.
